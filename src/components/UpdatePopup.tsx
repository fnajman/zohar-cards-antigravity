      ...
      {showPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-night/80 backdrop-blur-sm">
          ...
          >
            {isUpdating ? (
              <>
                <div className="w-12 h-12 rounded-full border-2 border-parchment/20 border-t-parchment animate-spin mb-4" />
                <h3 className="text-lg font-medium text-parchment mb-2">
                  {t('update.updating_title', 'Mise à jour en cours')}
                </h3>
                <p className="text-sm text-ash mb-2 leading-relaxed">
                  {t('update.updating_desc', 'Veuillez patienter...')}
                </p>
              </>
            ) : (
              <>
